...
class eth_mon extends uvm_monitor;
            ...
            statistics::v_uif[mac_addr].tx_bad_pkt_count++;     
              ...
              if(tr.priority_en_vector[i]) begin
                if(tr.pfc_pause_time[i] == 0) begin
                  ...
                  statistics::v_uif[mac_addr].tx_pfc_xon_prio_count[i]++;
                end
                else begin
                  statistics::v_uif[mac_addr].tx_pfc_xoff_count++;
                  statistics::v_uif[mac_addr].tx_pfc_xoff_prio_count[i]++;
                end
              end
            end
	        end
            ...
            statistics::v_uif[mac_addr].rx_good_pkt_count++;	
           	...
           	for(int i=0;i<8;i++) begin
                ...
                if(tr.pfc_pause_time[i] == 0) begin
                  ...
                  statistics::v_uif[mac_addr].rx_pfc_xon_prio_count[i]++;
                end
                else begin
                  statistics::v_uif[mac_addr].rx_pfc_xoff_count++;
                  statistics::v_uif[mac_addr].rx_pfc_xoff_prio_count[i]++;
                end
      ...
      if(tr.ether_type <= 16'd1500 && !tr.pause_frame_en && !tr.pfc_frame_en) begin
        ...
        if(payload_size < min_payload) begin
          ...
          if(payload_size != actual_payload_size) begin
            len_mismatch = 1;
            `uvm_error("MON_LEN_MISMATCH", $sformatf( "DA=%h SA=%h claimed=%0d actual=%0d", tr.da, tr.sa, payload_size,
             actual_payload_size))
          end
        end
        end
  ...
  function void reset_counters();
    ...
    for(int i=0;i<8;i++) begin
      statistics::v_uif[mac_addr].tx_pfc_xon_prio_count[i]  = 0;
      statistics::v_uif[mac_addr].tx_pfc_xoff_prio_count[i] = 0;
 
      statistics::v_uif[mac_addr].rx_pfc_xon_prio_count[i]  = 0;
      statistics::v_uif[mac_addr].rx_pfc_xoff_prio_count[i] = 0;
    end
  endfunction
  ...
  function void report_phase(uvm_phase phase);
    ...
    pfc_report = "\n";
    for(int i=0;i<8;i++) begin
      pfc_report = {pfc_report, $sformatf( "PCP[%0d] : TX_XON=%0d TX_XOFF=%0d RX_XON=%0d RX_XOFF=%0d\n", i,
            statistics::v_uif[mac_addr].tx_pfc_xon_prio_count[i],
            statistics::v_uif[mac_addr].tx_pfc_xoff_prio_count[i],
            statistics::v_uif[mac_addr].rx_pfc_xon_prio_count[i],
            statistics::v_uif[mac_addr].rx_pfc_xoff_prio_count[i]
     ...
     `uvm_info("PFC_REPORT", $sformatf( "\n---------------- MAC %0d : PFC PRIORITY COUNTERS ----------------\n%s", mac_no(mac_addr),
          pfc_report), UVM_NONE) 
  endfunction
endclass

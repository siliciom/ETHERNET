...
class virtual_seq extends base_virtual_seq;
  ...
  task body();
    ...
    `else
      this.mode = 1;
    `endif
    `uvm_info("",$sformatf("---------- MODE = %0d -----------",this.mode),UVM_LOW);
	...
	repeat(this.no_of_pkts) begin
	  ...
	  if(pkt_gap_cnt >0)
	    pkt_gap_cnt--;
	  //Basic_pfc  
	  if(basic_pfc_en && $urandom_range(1,50)<10 && total_no_of_pkts_0 < (no_of_pkts-30)) begin
	    seq1.pfc_sel=1;    
      ...
      //---------------- Multi Priority PFC ----------------
      else if(multi_priority_pfc_en &&
              (count != 0) &&
              ((count+1)%20==0) && total_no_of_pkts_0 < (no_of_pkts-30)) begin

        int pri[$];
                  seq1.pfc_sel = 1;

        // Give quanta to all priorities
        foreach(seq1.pfc_pause_time[i])
          seq1.pfc_pause_time[i] = $urandom_range(6,9);

        // Clear enable vector
        seq1.priority_en_vector = '0;
	  ...
	  //random_priority
	  else if(pfc_rand_pri_en && $urandom_range(1,100)<=10 && total_no_of_pkts_0 < (no_of_pkts-30)) begin
	    seq1.pfc_sel=1;
	  ...
	  //Simulataneous_pfc
	  else if(pfc_simul_en && $urandom_range(1,50) <=10 && total_no_of_pkts_0 < (no_of_pkts-30)) begin
	    seq1.pfc_sel=1;
	    assert(pcp_gen.randomize());
	  ...
	  else if(back_to_back_xoff_xon_en &&(waiting_for_xon || $urandom_range(1,100)<10) && pkt_gap_cnt==0 && total_no_of_pkts_0 < (no_of_pkts-30)) begin
	    seq1.pfc_sel = 1;
	  ...
	  else if(pfc_overlap_en && total_no_of_pkts_0 < (no_of_pkts-30)) begin //independent_priority_overlap
	    if(!vlan_phase) begin
	  ...
	  else if(pfc_stress_en && (seq1.pfc_sel || pcp_rand_en) && total_no_of_pkts_0 < (no_of_pkts-30)) begin
	  ...
	  else if(multiple_pfc_stress_en && count >=2 && count <=9 && total_no_of_pkts_0 < (no_of_pkts-30)) begin      
	...
	if(simul_pfc_en2 && total_no_of_pkts_1 < (no_of_pkts-30)) begin
	  seq2.pfc_sel=1;
	  assert(pcp_gen.randomize());
